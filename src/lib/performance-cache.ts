...
class PerformanceCache<T> {
  ...
  constructor(_name: string, maxSize = 1000, ttlMs: number = 2 * 60 * 1000) {
...
export const paymentCache = new PerformanceCache<unknown>(
  "payments",
  1000,
  5 * 60 * 1000
); // 5 min, 1000 users
export const paymentExpirationCache = new PerformanceCache<unknown>(
  "payment-expiration",
  1000,
  30 * 60 * 1000
); // 30 min, 1000 users
export const dodoProStatusCache = new PerformanceCache<unknown>(
  "dodo-pro-status",
  1000,
  30 * 60 * 1000
); // 30 min, 1000 users
...
export const createPaymentExpirationKey = (userId: string) =>
  `payment-expiration:${userId}`;
export const createDodoProStatusKey = (userId: string) =>
  `dodo-pro-status:${userId}`;
...
export function extractSessionToken(headers: Headers): string | null {
  ...
  const match = cookies.match(SESSION_TOKEN_REGEX);
...
export function setDodoPayments(userId: string, payments: unknown) {
  const cacheKey = createPaymentKey(userId);
...
export function setDodoPaymentExpiration(
  userId: string,
  expirationData: unknown
) {
  const cacheKey = createPaymentExpirationKey(userId);
...
export function setDodoProStatus(userId: string, statusData: unknown) {
  const cacheKey = createDodoProStatusKey(userId);
